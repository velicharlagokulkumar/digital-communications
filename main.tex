...
\begin{aligned}
& x(t)=s(t)+w(t) \\
\Rightarrow & \left\langle x(t), \phi_i(t)\right\rangle=\left\langle s(t), \phi_i(t)\right\rangle+\left\langle w(t), \phi_i(t)\right\rangle \\
\end{aligned}
$$
similarly Here,   \begin{equation}
    \mbf{y} = A\mbf{x} + \mbf{n},
    \end{equation}
    \begin{align}
            \myvec{y_1 \\ y_2}=A*Either \myvec{1 \\ 0} or \myvec{0 \\ 1}+\myvec{n_1 \\ n_2} \\
             \myvec{y_1 \\ y_2}=Either \myvec{A \\ 0} or \myvec{0 \\ A}+\myvec{n_1 \\ n_2}
    \end{align}

$$
\begin{aligned}
\Rightarrow & \hat{m}=\arg \max \left\{P\left(\boldsymbol{y} \mid\left(A \ 0\right)^T\right), P\left(\boldsymbol{y} \mid\left(0 \  A \right)^T\right)\right\} \\
\Rightarrow & y_1-y_2 \underset{\hat{m}=s_1}{\stackrel{\hat{m}=s_0}{\lessgtr}} 0  
\end{aligned}
$$
%\begin{equation}
 %    \quad\left(x_1-x_2\right)=\pm \sqrt{E_b}+\left(w_1-w_2\right)
%\end{equation}
\begin{equation}
\therefore y_1 \underset{s_1}{\stackrel{s_0}{\lessgtr}} y_2
\end{equation}
